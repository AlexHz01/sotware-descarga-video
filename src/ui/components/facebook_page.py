...
class FacebookPage(ctk.CTkFrame):
    def __init__(self, master, downloader_logic, config):
        ...
        # --- Cabecera ---
        self.label_title = ctk.CTkLabel(self, text="Descargador de Facebook", font=("Roboto", 24, "bold"), text_color="#1877F2")
        ...
        self.entry_url = ctk.CTkEntry(self.frame_url, placeholder_text="Pega el link de Facebook aquí...", height=45)
        self.entry_url.grid(row=0, column=0, padx=(20, 10), pady=20, sticky="ew")
        self.entry_url.bind("<KeyRelease>", self.on_url_change)
        
        self.btn_search = ctk.CTkButton(self.frame_url, text="Buscar", width=100, height=45, fg_color="#1877F2", hover_color="#145DB2", command=self.fetch_video_info)
        ...
        self.btn_download_now = ctk.CTkButton(self.frame_actions, text="DESCARGAR AHORA", height=50, font=("Roboto", 16, "bold"),
                                            fg_color="#1877F2", hover_color="#145DB2", command=self.download_now)
        self.btn_download_now.grid(row=0, column=1, padx=(10, 0), sticky="ew")
        
        # --- Cola de Descarga ---
        self.queue_frame = ctk.CTkScrollableFrame(self, height=150, label_text="Cola de Facebook")
        self.queue_frame.grid(row=5, column=0, sticky="ew", pady=(10, 20))
        self.queue_frame.grid_columnconfigure(0, weight=1)

        # --- Progreso y Estado ---
        self.progress_bar = ctk.CTkProgressBar(self, progress_color="#1877F2")
    ...
    def fetch_video_info(self):
        ...
        self.info_label.configure(text="Buscando video en Facebook...")
        ...
        def task():
            ...
            if info:
                ...
                self.after(0, lambda: self.info_label.configure(text="No se pudo obtener la información de Facebook.\nVerifica que el video sea público."))
            ...
            def progress(d):
                if d['status'] == 'downloading':
                    ...
                    try:
                        ...
                        self.after(0, lambda: self.status_label.configure(text=f"Descargando FB: {p_str}"))
                    except: pass
