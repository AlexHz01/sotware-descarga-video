...
class DrivePage(ctk.CTkFrame):
    def __init__(self, master, downloader_logic, config):
        ...
        # --- Cabecera ---
        self.label_title = ctk.CTkLabel(self, text="Descargador de Google Drive", font=("Roboto", 24, "bold"), text_color="#34A853")
        ...
        self.entry_url = ctk.CTkEntry(self.frame_url, placeholder_text="Pega el link de Google Drive aquí...", height=45)
        self.entry_url.grid(row=0, column=0, padx=(20, 10), pady=20, sticky="ew")
        self.entry_url.bind("<KeyRelease>", self.on_url_change)
        
        self.btn_search = ctk.CTkButton(self.frame_url, text="Buscar", width=100, height=45, fg_color="#34A853", hover_color="#2D8F47", command=self.fetch_video_info)
        ...
        self.btn_download_now = ctk.CTkButton(self.frame_actions, text="DESCARGAR AHORA", height=50, font=("Roboto", 16, "bold"),
                                            fg_color="#34A853", hover_color="#2D8F47", command=self.download_now)
        self.btn_download_now.grid(row=0, column=1, padx=(10, 0), sticky="ew")
        
        # --- Cola de Descarga ---
        self.queue_frame = ctk.CTkScrollableFrame(self, height=150, label_text="Cola de Drive")
        self.queue_frame.grid(row=5, column=0, sticky="ew", pady=(10, 20))
        self.queue_frame.grid_columnconfigure(0, weight=1)

        # --- Progreso y Estado ---
        self.progress_bar = ctk.CTkProgressBar(self, progress_color="#34A853")
    ...
    def fetch_video_info(self):
        ...
        self.info_label.configure(text="Buscando archivo en Google Drive...")
        ...
        def task():
            ...
            if info:
                ...
                self.after(0, lambda: self.info_label.configure(text="No se pudo obtener la información de Drive.\nVerifica que el archivo esté compartido públicamente."))
        
        self.fetching_thread = threading.Thread(target=task)
        self.fetching_thread.start()

    def update_preview(self, info):
        title = info.get('title', 'Archivo de Drive')
        uploader = info.get('uploader', 'Drive User')
        
        text = f"Título: {title[:50]}...\nUploader: {uploader}"
        self.info_label.configure(text=text)
        
        # Drive a veces no tiene thumbnail accesible fácilmente vía yt-dlp sin auth
        self.after(0, lambda: self.img_label.configure(text="Drive File"))
    ...
    def add_queue_item_ui(self, item):
        ...
        title = item['info'].get('title', 'Video')[:40]
    ...
    def process_queue(self):
        ...
        while self.download_queue:
            ...
            def progress(d):
                if d['status'] == 'downloading':
                    ...
                    try:
                        ...
                        self.after(0, lambda: self.status_label.configure(text=f"Descargando Drive: {p_str}"))
                    except: pass
            ...
            self.downloader_logic.download(url, path, "MP4", "best")
            done_event.wait()
        ...
        self.is_downloading = False
        self.after(0, lambda: self.status_label.configure(text="Bajadas completadas"))
